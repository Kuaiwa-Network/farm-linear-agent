...
2. Claim first: `python3 -m agent --db DATABASE claim --item ITEM_ID --worker-id WORKER_ID`. Write the
   token to `.local/runs/ITEM_ID/token` with mode 0600 and pass `--item ITEM_ID --token-file .local/runs/ITEM_ID/token`
   ...
   `python3 -m agent --db DATABASE renew --item ITEM_ID --token-file .local/runs/ITEM_ID/token`; your
...
5. Post the answer as a session activity: `python3 -m agent --db DATABASE activity --item ITEM_ID --token-file .local/runs/ITEM_ID/token --type response --body-file ANSWER.md`.
   ...
   `python3 -m agent --db DATABASE await-input --item ITEM_ID --token-file .local/runs/ITEM_ID/token --question TEXT` and exit.
